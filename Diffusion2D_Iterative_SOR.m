...
while (residual > epsilon)  
    for i = 2:i_max-1
        for j = 2:j_max-1
            ...
            % overrelaxation: α > 1.0   --Increases rate of convergence, too large will cause unstable
            T(i,j) = T(i,j) + alpha*( (T(i+1,j)-2*T(i,j)+T(i-1,j))/Deltax^2 + (T(i,j+1)-2*T(i,j)+T(i,j-1))/Deltay^2 ) / (2.0/Deltax^2 + 2.0/Deltay^2);
        end
    end
    iter = iter + 1;
